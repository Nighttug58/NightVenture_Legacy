(function () {
    ...
    const NV_REST_INN_VERSION =
        "v0.9.7.2-rest-inn-compact-contextual";
    ...
    function NVR_isSafeZone(zone) {
        ...
        return (
            ...
            /auberge|sanctuaire|poste|marché|marche|tour/i.test(zone.nom || "")
        );
    }
    ...
    function NVR_percent(valeur, maximum) {
        return maximum > 0
            ? Math.max(0, Math.min(100, (Number(valeur || 0) / maximum) * 100))
            : 0;
    }
    ...
    function NVR_costRest() {
        ...
        const missing =
            NVR_missingResources();

        const niveau =
            Math.max(1, Number(personnage.niveau || 1));

        const missingTotal =
            missing.pv + missing.mana + missing.stamina;

        return Math.max(
            1,
            Math.round(
                NV_REST_CONFIG.baseCost +
                niveau * NV_REST_CONFIG.costPerLevel +
                missingTotal / NV_REST_CONFIG.missingResourceDivider
            )
        );
    }
    ...
    function NVR_majBarresHautSansRedraw() {
        ...
        const ressources = [
            {
                selecteur: ".barre-pv",
                valeur: Number(personnage.pv || 0),
                max: NVR_pvMax()
            },
            {
                selecteur: ".barre-mana",
                valeur: Number(personnage.mana || 0),
                max: NVR_manaMax()
            },
            {
                selecteur: ".barre-stamina",
                valeur: Number(personnage.stamina || 0),
                max: NVR_staminaMax()
            },
            {
                selecteur: ".barre-xp",
                valeur: Number(personnage.xp || 0),
                max: NVR_xpMax()
            }
        ];
        ...
        ressources.forEach(ressource => {
            ...
            const pourcentage =
                NVR_percent(ressource.valeur, ressource.max);
        ...
        if (typeof NVS_injecterLabelsRegenDOM === "function") {
    ...
    function NVR_majInfosMondeSansRedraw() {
        ...
        const region =
            NVR_regionActuelle()?.nom || "Région inconnue";

        const zone =
            NVR_zoneActuelle()?.nom || "Zone inconnue";
        ...
        const heureAffichee =
            `${heure}h${String(minute).padStart(2, "0")}`;
    ...
    function NVR_majPersonnageMinimalSansRedraw() {
        ...
        if (typeof NVBU_mettreAJourBoutonExplorer === "function") {
    ...
    function NVR_seReposer() {
        ...
        if (!NVR_isExplorationView()) {
            NVR_retirerCardRepos();
            return;
        }

        if (!NVR_isSafeZone(zone)) {
            NVR_journal("🛏 Tu dois être dans une zone sûre pour te reposer.");
            NVR_retirerCardRepos();
            return;
        }
        ...
        if (NVR_estDejaFull()) {
            NVR_journal("🛏 Tu es déjà parfaitement reposé.");
            NVR_majCardRepos();
            return;
        }

        personnage.or =
            Math.max(0, Number(personnage.or || 0) - cout);

        NVR_restaurerRessources();

        NVR_avancerTempsSansRefresh(NV_REST_CONFIG.restHours, 0);

        NV_REST_STATE.dernierRepos =
            {
                zoneId: zone?.id,
                cout,
                jour: personnage.jour,
                heure: personnage.heure,
                minute: personnage.minute
            };

        NVR_journal(`🛏 Repos à ${zone?.nom || "l'auberge"} : ressources restaurées. -${cout} or.`);

        /*
            Pas de rafraichirInterface(), pas de ouvrirExploration().
            Mise à jour légère uniquement.
        */
        NVR_majPersonnageMinimalSansRedraw();
        NVR_majCardRepos();

        if (typeof NV_demanderAutosave === "function") {
            NV_demanderAutosave("rest inn compact no-redraw");
        }
    }
    ...
    function NVR_htmlCard() {
        ...
        if (!NVR_isExplorationView() || !NVR_isSafeZone(zone)) return "";
        ...
        return `
            <div id="nvRestInnCard" class="nv-rest-inn-compact">
                ...
                <div class="nv-rest-inn-compact__actions">
                    ...
                    <button
                        id="nvRestInnButton"
                        class="nv-rest-inn-button"
                        onclick="NVR_seReposer()"
                    >
                        Se reposer
                    </button>
                </div>
    ...
    function NVR_creerOuTrouverCardRepos() {
        ...
        let card =
            document.getElementById("nvRestInnCard");
        ...
        /*
            Placement voulu :
            juste après la carte de zone principale,
            donc avant PNJ / marchands / créatures / exploration.
        */
        const premiereCarte =
            conteneur.querySelector(".item-card");

        if (premiereCarte) {
            premiereCarte.insertAdjacentHTML("afterend", html);
        } else {
            conteneur.insertAdjacentHTML("afterbegin", html);
        }
    ...
    function NVR_majCardRepos() {
        ...
        const or =
            Number(Game.data.personnage.or || 0);

        const possible =
            !full && or >= cout;

        const costNode =
            document.getElementById("nvRestInnCost");

        const button =
            document.getElementById("nvRestInnButton");

        const summary =
            document.getElementById("nvRestInnSummary");
    ...
    function NVR_patchOuvrirExploration() {
        if (typeof ouvrirExploration !== "function" || ouvrirExploration.__NVR_0972_PATCH) return;
        ...
        ouvrirExploration.__NVR_0972_PATCH =
            true;
    }

    function NVR_patchAfficherVuePrincipale() {
        if (typeof afficherVuePrincipale !== "function" || afficherVuePrincipale.__NVR_0972_PATCH) return;
        ...
        afficherVuePrincipale = function (html) {
            ...
            /*
                Sécurité : si on change de page, on retire immédiatement
                la barre de repos au lieu de la laisser en permanence.
            */
            setTimeout(NVR_majCardRepos, 0);
        ...
        afficherVuePrincipale.__NVR_0972_PATCH =
            true;
    }

    function NVR_demarrerTick() {
        if (NV_REST_STATE.intervalId) {
            clearInterval(NV_REST_STATE.intervalId);
        }

        NV_REST_STATE.intervalId =
            setInterval(NVR_majCardRepos, NV_REST_CONFIG.tickMs);
    }
        ...
        style.textContent =
            `
                .nv-rest-inn-compact {
                    ...
                    background:
                        radial-gradient(circle at top right, rgba(245, 211, 122, 0.06), transparent 42%),
                ...
                .nv-rest-inn-compact__main strong {
                    color: var(--gold, #f5d37a);
                    font-size: 0.94rem;
                    line-height: 1.1;
                }

                .nv-rest-inn-compact__main span {
                ...
                .nv-rest-inn-button:disabled {
                    opacity: 0.55;
                    cursor: not-allowed;
                    filter: grayscale(0.25);
                }

                .nv-rest-inn-compact--ready {
                    border-color: rgba(80, 220, 130, 0.22);
                    background:
                        radial-gradient(circle at top right, rgba(80, 220, 130, 0.06), transparent 42%),
                        rgba(255,255,255,0.032);
                }

                .nv-rest-inn-compact--ready .nv-rest-inn-compact__main strong {
                    color: #9dffb8;
                }

                .nv-rest-inn-compact--disabled {
                    opacity: 0.86;
                }
    ...
    function NVR_installer() {
        ...
        console.log(`✅ Rest_Inn.js chargé — ${NV_REST_INN_VERSION}`);
    }

    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", NVR_installer);
    } else {
        NVR_installer();
    }

    window.NV_REST_INN_VERSION =
        NV_REST_INN_VERSION;

    window.NV_REST_CONFIG =
        NV_REST_CONFIG;

    window.NV_REST_STATE =
        NV_REST_STATE;

    window.NVR_seReposer =
        NVR_seReposer;

    window.NVR_majCardRepos =
        NVR_majCardRepos;
})();
