/* ============================================================================
   NIGHTVENTURE — COMBAT
   Moteur de combat joueur : initiative, actions, effets, IA simple, résumé.
   ============================================================================ */
...
function enregistrerDegatsCombat(source, resultat) {
    const combat = Game.combat.actif;
    if (!combat?.stats || !source || !resultat) return;
...
function enregistrerSoinCombat(cible, valeur) {
    ...
    if (cible.camp === "joueur") {
        combat.stats.soinsJoueur += soin;
    } else {
        combat.stats.soinsEnnemi += soin;
    }
}
...
function enregistrerTentativeFuiteCombat(reussie) {
    ...
    combat.stats.fuiteReussie = Boolean(reussie);
}
...
function obtenirCompetencesCombatJoueur() {
    ...
    const competencesBase = ["attaque_simple", "defendre", "fuir", "utiliser_objet"];
    const competencesClasse = Array.isArray(classe?.competencesDepart) ? classe.competencesDepart : [];

    return [...new Set([...competencesBase, ...competencesClasse])].filter(Boolean);
}
...
function obtenirCompetencesCombatMonstre(monstre) {
    ...
    if (liste.length > 0) {
        return liste
            .map(entree => typeof entree === "string" ? entree : entree.id)
            .filter(Boolean);
    }
...
function appliquerEffetsDebutTour(combattant) {
    ...
    for (const effet of [...combattant.effets]) {
        ...
        if (effet.type === "hot") {
            ...
            enregistrerSoinCombat(combattant, soin);
    ...
    switch (action.typeAction) {
        case "attaque": {
            ...
            if (resultat.esquive) {
                ...
                enregistrerDegatsCombat(source, resultat);
                ligne = `${source.camp === "joueur" ? "⚔ Joueur" : "⚔ " + source.nom} utilise ${action.nom} et inflige ${resultat.degats} dégâts à ${cible.nom}${resultat.critique ? " (critique)" : ""}.`;
            }
        ...
        case "soin": {
            ...
            enregistrerSoinCombat(source, soin);
            ligne = `💚 ${source.nom} récupère ${soin} PV avec ${action.nom}.`;
            break;
        }
        ...
        case "fuite": {
            ...
            ligne = `❌ Vous échouez à fuir (${chance}% de chance).`;
            break;
        }

        case "objet": {
            ligne = "⚠ Sélectionnez directement une potion utilisable.";
            break;
        }
...
function utiliserPotionCombat(idObjet) {
    ...
    if (!objet.soin) {
        ajouterLigneCombat(`⚠ ${objet.nom} n'a pas d'effet utilisable en combat.`);
        ouvrirCombat();
        return;
    }

    enregistrerActionCombat(combat.joueur, {
        id: `potion_${objet.id}`,
        nom: objet.nom
    });

    combat.joueur.pv = Math.min(combat.joueur.pvMax, combat.joueur.pv + objet.soin);
    retirerObjetInventaire(idObjet, 1);
    enregistrerPotionCombat(objet.soin);
    ajouterLigneCombat(`🧪 Vous utilisez ${objet.nom} et récupérez ${objet.soin} PV.`);

    combat.joueur.initiative = Math.max(0, combat.joueur.initiative - 90);
    appliquerEffetsFinTour(combat.joueur);
    synchroniserJoueurDepuisCombat();
    avancerInitiativeCombat();
    ouvrirCombat();
}
    ...
    return `
        <div class="item-card resume-combat">
            ...
            <div class="resume-combat__grille">
                <div class="resume-combat__section">
                    <h4>⏱ Général</h4>
                    <div class="resume-combat__ligne"><span>Durée</span><strong>${formaterDureeCombat(stats.dureeMs)}</strong></div>
                ...
                <div class="resume-combat__section">
                    <h4>⚔ Dégâts</h4>
                    <div class="resume-combat__ligne"><span>Dégâts infligés</span><strong>${stats.degatsInfliges}</strong></div>
                    <div class="resume-combat__ligne"><span>Dégâts reçus</span><strong>${stats.degatsRecus}</strong></div>
                ...
                <div class="resume-combat__section">
                    ...
                    <div class="resume-combat__ligne"><span>Défenses joueur</span><strong>${stats.defensesJoueur}</strong></div>
                    <div class="resume-combat__ligne"><span>Défenses ennemi</span><strong>${stats.defensesEnnemi}</strong></div>
                ...
                <div class="resume-combat__section">
                    ...
                    <div class="resume-combat__ligne"><span>Potions utilisées</span><strong>${stats.potionsUtilisees}</strong></div>
                    <div class="resume-combat__ligne"><span>Tentatives fuite</span><strong>${stats.tentativesFuite}</strong></div>
                    <div class="resume-combat__ligne"><span>Fuite réussie</span><strong>${stats.fuiteReussie ? "Oui" : "Non"}</strong></div>
                </div>
...
function ouvrirCombat() {
    ...
    if (combat.statut !== "en_cours") {
        let html = `
            <div class="item-card">
                ...
                <p>Le combat contre <strong>${combat.ennemi.nom}</strong> est terminé.</p>
            </div>
        `;

        html += creerResumeCombat(combat);
        html += `
            <div class="item-card">
                <button onclick="quitterCombat()">⬅ Retour à l'exploration</button>
            </div>
        `;
    ...
    const ennemi = combat.ennemi;
    const peutAgir = combat.phase === "joueur" && combat.statut === "en_cours";

    let html = `
        <div class="item-card">
            <h2>⚔ Combat</h2>
            <p>Tour ${combat.tour} — ${combat.phase === "joueur" ? "À vous d'agir" : "Tour ennemi"}</p>
        </div>

        <div class="item-card">
            <h3>🧙 ${joueur.nom}</h3>
            ${creerBarreRessource("barre-pv", calculerPourcentage(joueur.pv, joueur.pvMax), joueur.pv, joueur.pvMax, "❤️ PV")}
            ${creerBarreRessource("barre-mana", calculerPourcentage(joueur.mana, joueur.manaMax), joueur.mana, joueur.manaMax, "🔵 Mana")}
            ${creerBarreRessource("barre-stamina", calculerPourcentage(joueur.stamina, joueur.staminaMax), joueur.stamina, joueur.staminaMax, "🟢 Stamina")}
            ${creerBarreInitiativeCombat("Initiative joueur", joueur)}
        </div>

        <div class="item-card">
            ${ennemi.image ? `<img src="${ennemi.image}" class="portrait-monstre">` : ""}
            <h3>👹 ${ennemi.nom}</h3>
            ${creerBarreRessource("barre-pv", calculerPourcentage(ennemi.pv, ennemi.pvMax), ennemi.pv, ennemi.pvMax, "❤️ PV")}
            ${creerBarreRessource("barre-mana", calculerPourcentage(ennemi.mana, ennemi.manaMax), ennemi.mana, ennemi.manaMax, "🔵 Mana")}
            ${creerBarreRessource("barre-stamina", calculerPourcentage(ennemi.stamina, ennemi.staminaMax), ennemi.stamina, ennemi.staminaMax, "🟢 Stamina")}
            ${creerBarreInitiativeCombat("Initiative ennemi", ennemi)}
        </div>

        <div class="item-card">
            <h3>🎮 Actions</h3>
            <div class="combat-actions">
                <button ${peutAgir ? "" : "disabled"} onclick="attaquerMonstre()">⚔ Attaquer</button>
                <button ${peutAgir ? "" : "disabled"} onclick="defendreCombat()">🛡 Défendre</button>
                <button ${peutAgir ? "" : "disabled"} onclick="fuirCombat()">🏃 Fuir</button>
                <button ${peutAgir ? "" : "disabled"} onclick="utiliserPotionCombat('potion_soin')">🧪 Potion</button>
            </div>
        </div>
    `;

    html += creerMiniJournalCombat(combat);
    afficherVuePrincipale(html);
}
